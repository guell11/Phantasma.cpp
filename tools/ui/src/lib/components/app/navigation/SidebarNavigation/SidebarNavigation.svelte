...
{#if innerWidth > 768 || (!page.url.hash.includes(ROUTES.SETTINGS) && !page.url.hash.includes(ROUTES.MCP_SERVERS) && !page.url.hash.includes(ROUTES.SEARCH))}
	...
	>
			<div class="px-2 flex items-center justify-between">
				{#if isExpandedMode && innerWidth > 768}
					<div class="pointer-events-none absolute left-13 top-3.5 flex items-baseline gap-1 whitespace-nowrap">
						<span class="text-sm font-semibold tracking-[-0.03em]">phantasma</span><span class="text-sm font-semibold text-cyan-300">.cpp</span>
						<span class="ml-1 rounded-full border border-violet-400/20 bg-violet-400/10 px-1.5 py-0.5 text-[9px] font-semibold tracking-[0.12em] text-violet-300 uppercase">local</span>
					</div>
				{/if}
				<div
				role="button"
