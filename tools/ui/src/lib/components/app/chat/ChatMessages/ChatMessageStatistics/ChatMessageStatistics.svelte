<script lang="ts">
	...
	let hasDraftStats = $derived(draftTokens !== undefined && draftTokens > 0);
	let draftAcceptance = $derived(
		hasDraftStats && draftAccepted !== undefined ? (draftAccepted / draftTokens!) * 100 : undefined
	);
</script>
		...
		{#if activeView === ChatMessageStatsView.GENERATION && hasGenerationStats}
			...
			{#if hasDraftStats}
				<ChatMessageStatisticsBadge
					class="bg-transparent"
					icon={GitBranch}
					value="{draftAccepted ?? 0}/{draftTokens} draft"
					tooltipLabel={draftAcceptance === undefined
						? 'Speculative draft tokens accepted / proposed'
						: `Speculative acceptance ${draftAcceptance.toFixed(1)}%`}
				/>
			{/if}
