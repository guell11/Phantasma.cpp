<script lang="ts">
	...
	/**
	 * Removes click event listeners from copy, download, and preview buttons.
	 ...
	 * Downloads the raw code content as a local file.
	...
	function handleDownloadClick(event: Event) {
		...
		downloadCodeBlock(info.rawCode, info.language);
	}
	...
	/**
	 * Attaches click event listeners to copy, download, and preview buttons in code blocks.
	...
	async function handleMermaidClick(event: MouseEvent) {
		...
		// Check if clicking on an action button in mermaid block
		const copyBtn = target.closest(`.${MERMAID_WRAPPER_CLASS} .copy-code-btn`);
		const downloadBtn = target.closest(`.${MERMAID_WRAPPER_CLASS} .download-code-btn`);
		const previewBtn = target.closest(`.${MERMAID_WRAPPER_CLASS} .preview-code-btn`);

		if (copyBtn || downloadBtn || previewBtn) {
			...
			if (downloadBtn) {
				event.preventDefault();
				event.stopPropagation();
				downloadCodeBlock(mermaidSyntax, MERMAID_LANGUAGE);
				return;
			}
		...
		// Check if clicking on an action button in svg block
		const svgCopyBtn = target.closest(`.${SVG_WRAPPER_CLASS} .copy-code-btn`);
		const svgDownloadBtn = target.closest(`.${SVG_WRAPPER_CLASS} .download-code-btn`);
		const svgPreviewBtn = target.closest(`.${SVG_WRAPPER_CLASS} .preview-code-btn`);

		if (svgCopyBtn || svgDownloadBtn || svgPreviewBtn) {
			...
			if (svgDownloadBtn) {
				event.preventDefault();
				event.stopPropagation();
				downloadCodeBlock(preElement.getAttribute(SVG_SOURCE_ATTR) ?? '', SVG_LANGUAGE);
				return;
			}
		...
		{#if incompleteCodeBlock.language === MERMAID_LANGUAGE}
			<div class="mermaid-block-wrapper streaming-mermaid-block">
				<div class="code-block-header">
					...
					<CodeBlockActions code={incompleteCodeBlock.code} disabled language={MERMAID_LANGUAGE} />
				</div>
		...
		{:else if streamingSvgCode !== null}
			<div class="svg-block-wrapper streaming-svg-block">
				<div class="code-block-header">
					...
					<CodeBlockActions code={incompleteCodeBlock.code} disabled language={SVG_LANGUAGE} />
				</div>
				{#if liveSvgHtml}
