<script lang="ts">
	...
	let speculativeParams = $derived(serverProps?.default_generation_settings?.params);
	let speculativeEnabled = $derived(serverProps?.default_generation_settings?.speculative ?? false);
	let modelExpertCount = $derived.by(() => {
		const value = firstModel?.meta?.n_expert;
		return typeof value === 'number' && value > 0 ? value : null;
	});
	let modelExpertsUsed = $derived.by(() => {
		const value = firstModel?.meta?.n_expert_used;
		return typeof value === 'number' && value > 0 ? value : null;
	});
					...
					<Table.Root>
						...
						<Table.Body>
							...
							{#if modalities.length > 0}
								...
								<Table.Row>
									<Table.Cell class="align-middle font-medium">Runtime</Table.Cell>
									<Table.Cell>
										{serverProps.default_generation_settings.is_processing ? 'Generating' : 'Idle'}
									</Table.Cell>
								</Table.Row>

								<Table.Row>
									<Table.Cell class="align-middle font-medium">Speculative Decoding</Table.Cell>
									<Table.Cell>{speculativeEnabled ? 'Enabled' : 'Disabled'}</Table.Cell>
								</Table.Row>

								{#if speculativeEnabled && speculativeParams}
									<Table.Row>
										<Table.Cell class="align-middle font-medium">Draft Window</Table.Cell>
										<Table.Cell>
											{speculativeParams['speculative.n_min']} - {speculativeParams[
												'speculative.n_max'
											]} tokens, p >= {speculativeParams['speculative.p_min']}
										</Table.Cell>
									</Table.Row>
								{/if}

								{#if modelExpertCount !== null}
									<Table.Row>
										<Table.Cell class="align-middle font-medium">MoE Experts</Table.Cell>
										<Table.Cell>
											{formatNumber(modelExpertCount)} total{modelExpertsUsed !== null
												? `, ${formatNumber(modelExpertsUsed)} active/token`
												: ''}
										</Table.Cell>
									</Table.Row>
								{/if}
