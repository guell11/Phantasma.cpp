...
int main() {
    ...
    const common_tree_draft_node seed[] = {
        ...
        { 0, 11, 1, -1.0f, 20, COMMON_TREE_DRAFT_NODE_FLAG_FRONTIER },
        { 0, 12, 1, -2.0f, 30, COMMON_TREE_DRAFT_NODE_FLAG_FRONTIER },
    };
    for (const auto & node : seed) assert(common_tree_draft_arena_append(&arena, node, cfg) == COMMON_TREE_DRAFT_ARENA_OK);
    ...
    const double cumulative[] = { -0.1, -1.0, -2.0 };
    assert(common_tree_draft_mass_prune(&arena, ranks, cumulative, 3, 0.2, 3,
                COMMON_TREE_DRAFT_SCORE_CUMULATIVE, COMMON_TREE_DRAFT_DEPTH_NEUTRAL) == COMMON_TREE_DRAFT_MASS_PRUNE_OK);
    ...
    assert(common_tree_draft_mass_prune(&arena2, ranks, cumulative, 3, 0.01, 1,
                COMMON_TREE_DRAFT_SCORE_CUMULATIVE, COMMON_TREE_DRAFT_DEPTH_NEUTRAL) == COMMON_TREE_DRAFT_MASS_PRUNE_OK);
    assert(arena2.frontier_count == 1 && arena2.frontier[0] == 1);

    assert(common_tree_draft_mass_prune(&arena2, nullptr, nullptr, 0, 0.0, 1,
                COMMON_TREE_DRAFT_SCORE_CUMULATIVE, COMMON_TREE_DRAFT_DEPTH_NEUTRAL) == COMMON_TREE_DRAFT_MASS_PRUNE_THRESHOLD);
    return 0;
}
