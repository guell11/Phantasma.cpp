...
llama_batch_allocr::llama_batch_allocr(uint32_t n_pos_per_embd) : n_pos_per_embd(n_pos_per_embd) {
    ...
    seq_pos_min_v.resize(LLAMA_MAX_SEQ, -1);
    seq_pos_max_v.resize(LLAMA_MAX_SEQ, -1);
    seq_pos_last_v.resize(LLAMA_MAX_SEQ, -1);
    seq_pos_count.resize(LLAMA_MAX_SEQ, 0);
...
bool llama_batch_allocr::init(
    ...
    for (int32_t i = 0; i < batch.n_tokens; ++i) {
        ...
        for (int32_t s = 0; s < batch.n_seq_id[i]; ++s) {
            ...
            const llama_pos cur_pos = batch.pos[i];
            if (seq_pos_count[s1] == 0) {
                seq_pos_min_v[s1] = cur_pos;
                seq_pos_max_v[s1] = cur_pos;
                seq_pos_last_v[s1] = cur_pos;
                seq_pos_count[s1]  = 1;
            } else {
                seq_pos_min_v[s1] = std::min(seq_pos_min_v[s1], cur_pos);
                seq_pos_max_v[s1] = std::max(seq_pos_max_v[s1], cur_pos);
                if (cur_pos != seq_pos_last_v[s1]) {
                    seq_pos_last_v[s1] = cur_pos;
                    ++seq_pos_count[s1];
                }
            }
    ...
    if (debug > 0) {
        ...
        for (int s0 = 0; s0 < (int) seq_pos_count.size(); ++s0) {
            if (seq_pos_count[s0] == 0) {
                continue;
            }
    ...
    if (n_pos_per_embd > 1) {
        ...
        for (uint32_t s = 0; s < n_seq_max; ++s) {
            ...
            if (seq_pos_max(s) - seq_pos_min(s) + 1 > (int) seq_pos_count[s]) {
...
llama_pos llama_batch_allocr::seq_pos_min(llama_seq_id seq_id) const {
    return seq_pos_count[seq_id] == 0 ? -1 : seq_pos_min_v[seq_id];
}

llama_pos llama_batch_allocr::seq_pos_max(llama_seq_id seq_id) const {
    return seq_pos_count[seq_id] == 0 ? -1 : seq_pos_max_v[seq_id];
}
...
llama_ubatch llama_batch_allocr::split_equal(uint32_t n_ubatch, bool sequential, uint32_t n_keep_tail) {
    ...
    std::vector<const idx_vec_t *> seq_idxs(n_seqs);
    for (uint32_t s = 0; s < n_seqs; ++s) {
        const auto it = seq_set_map.find(cur_seq_set[s]);
        GGML_ASSERT(it != seq_set_map.end());
        seq_idxs[s] = &it->second;
    }
    ...
    for (uint32_t s = 0; s < n_seqs; ++s) {
        while (used[(*seq_idxs[s])[cur_idx[s]]]) {
            ++cur_idx[s];
        }
    }
    ...
    while (true) {
        ...
        for (uint32_t s = 0; s < n_seqs; ++s) {
            if (cur_idx[s] >= (int32_t) seq_idxs[s]->size()) {
                can_expand = false;
                break;
            }
        }
        ...
        for (uint32_t s = 0; s < n_seqs; ++s) {
            const int32_t idx = (*seq_idxs[s])[cur_idx[s]];
    ...
    if (n_keep_tail > 0) {
        ...
        auto n_remaining = [&](uint32_t s) {
            return (uint32_t) (seq_idxs[s]->size() - cur_idx[s]);
        };
...
void llama_batch_allocr::clear() {
    ...
    std::fill(seq_pos_last_v.begin(), seq_pos_last_v.end(), -1);
    std::fill(seq_pos_count.begin(), seq_pos_count.end(), 0);
