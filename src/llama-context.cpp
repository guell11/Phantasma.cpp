...
llama_context::llama_context(
    ...
    llama_expert_tier::context_constructing(model);
...
void llama_context::synchronize() {
    ...
    const bool tier_stats = llama_expert_tier::stats_enabled() &&
        n_queued_tokens > 0 && t_compute_start_us > 0;
    const int64_t t_sync_start_us = tier_stats ? ggml_time_us() : 0;
    ggml_backend_sched_synchronize(sched.get());
    const int64_t t_sync_end_us = tier_stats ? ggml_time_us() : 0;
    ggml_backend_t promotion_backend = nullptr;
    for (const auto & backend : backends) {
        if (ggml_backend_dev_type(ggml_backend_get_device(backend.get())) == GGML_BACKEND_DEVICE_TYPE_GPU) {
            promotion_backend = backend.get();
            break;
        }
    }
    const bool tier_updated = llama_expert_tier::update(model, promotion_backend, n_queued_tokens);
    const int64_t t_tier_end_us = tier_stats ? ggml_time_us() : 0;
    if (tier_stats && tier_updated) {
        llama_expert_tier::record_perf(
                n_queued_tokens,
                (uint64_t) (t_tier_end_us - t_compute_start_us),
                (uint64_t) (t_sync_end_us - t_sync_start_us),
                (uint64_t) (t_tier_end_us - t_sync_end_us));
    }
