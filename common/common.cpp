...
llama_model * common_init_result::release_model() {
    return pimpl->model.release();
}
...
struct llama_context_params common_context_params_to_llama(const common_params & params) {
    ...
    // The transitional Tree-MTP verifier keeps user-visible server slots unchanged and
    // reserves private sequence ids for branch verification. Each slot needs at most
    // one sibling branch per draft depth in the current comb-tree bring-up path.
    cparams.n_seq_max         = params.speculative.has_tree_mtp()
        ? params.n_parallel * 2
        : params.n_parallel;
