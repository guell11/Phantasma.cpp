...
enum common_speculative_type {
    ...
    COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE,// Tree-MTP speculative decoding (transitional branch verifier)
...
struct common_params_speculative {
    ...
    uint32_t need_n_rs_seq() const {
        bool needs_rs_seq = std::any_of(types.begin(), types.end(), [&](auto t) {
            return t == COMMON_SPECULATIVE_TYPE_DRAFT_MTP || t == COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE || t == COMMON_SPECULATIVE_TYPE_DRAFT_EAGLE3 || t == COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH;
        });

        return needs_rs_seq ? draft.n_max : 0u;
    }

    bool has_tree_mtp() const {
        return std::find(types.begin(), types.end(), COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE) != types.end();
    }
};
