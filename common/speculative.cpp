...
const std::map<std::string, common_speculative_type> common_speculative_type_from_name_map = {
    ...
    {"draft-mtp-tree",COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE},
...
struct common_speculative_impl {
    ...
    virtual void accept_at(llama_seq_id seq_id, uint16_t n_accepted, bool is_other, int32_t /*verify_batch_index*/) {
...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
    ...
    // Tree mode needs the hidden row of the branch actually selected by the
    // target verifier, not merely the same-depth trunk row.
    std::vector<float> verify_h_batch;
    int32_t verify_h_batch_rows = 0;
    ...
    common_speculative_impl_draft_mtp(const common_params_speculative & params, uint32_t n_seq, bool tree_mode = false)
        : common_speculative_impl(tree_mode ? COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE : COMMON_SPECULATIVE_TYPE_DRAFT_MTP, n_seq)
        , params(params.draft)
        , tree_mode(tree_mode)
    {
        auto * ctx_tgt = this->params.ctx_tgt;
    ...
    bool process(const llama_batch & batch_in) override {
        ...
        for (int k = 0; k < n_tokens; ++k) {
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                bool has_seq = false;
                for (int p = 0; p < batch_in.n_seq_id[k]; ++p) {
                    if (batch_in.seq_id[k][p] == seq_id) {
                        has_seq = true;
                        break;
                    }
                }
        ...
        if (tree_mode) {
            verify_h_batch_rows = n_tokens;
            verify_h_batch.resize((size_t) n_tokens * n_embd);
            for (int32_t k = 0; k < n_tokens; ++k) {
                const float * h = llama_get_embeddings_nextn_ith(ctx_tgt, k);
                if (h) {
                    std::memcpy(verify_h_batch.data() + (size_t) k * n_embd, h, row_bytes);
                }
            }
        }
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        int n_drafting = 0;
        std::vector<bool> drafting(n_seq);
        std::vector<int32_t> tree_parent(n_seq, -1);
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            if (tree_mode && dp.tree) {
                dp.tree->clear();
                dp.tree->nodes.reserve((size_t) std::max(0, params.n_max) * 2);
            }
        ...
        while (n_drafting > 0) {
            ...
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                ...
                if (tree_mode && dp.tree) {
                    const int32_t depth = (int32_t) result.size() + 1;
                    const int32_t parent = tree_parent[seq_id];

                    // Trunk node: exact token used to continue the MTP chain.
                    const int32_t trunk_index = (int32_t) dp.tree->nodes.size();
                    dp.tree->nodes.push_back({ id, parent, depth, cur_p->data[0].p });
                    tree_parent[seq_id] = trunk_index;

                    // One alternate sibling per depth keeps branch count bounded by n_max
                    // while exercising genuine branch-per-seq target verification.
                    if (cur_p->size > 1 && cur_p->data[1].id != id && cur_p->data[1].p > params.p_split) {
                        dp.tree->nodes.push_back({ cur_p->data[1].id, parent, depth, cur_p->data[1].p });
                    }
                }
    ...
    void accept_at(llama_seq_id seq_id, uint16_t n_accepted, bool is_other, int32_t verify_batch_index) override {
        if (!tree_mode || is_other || seq_id < 0 || seq_id >= (llama_seq_id) n_seq ||
                verify_batch_index < 0 || verify_batch_index >= verify_h_batch_rows) {
            accept(seq_id, n_accepted, is_other);
            return;
        }
        ...
        const size_t row_bytes = (size_t) n_embd * sizeof(float);
        std::memcpy(pending_h[seq_id].data(),
                verify_h_batch.data() + (size_t) verify_batch_index * n_embd,
                row_bytes);
    }
...
std::string common_speculative_type_to_str(common_speculative_type type) {
    switch (type) {
        ...
        case COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE:return "draft-mtp-tree";
...
int32_t common_speculative_n_max(const common_params_speculative * spec) {
    ...
    for (const auto type : spec->types) {
        switch (type) {
            ...
            case COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE:
...
common_params common_base_params_to_speculative(const common_params & params) {
    ...
    if (has_draft) {
        ...
        // The target model may have been mutated by --fit / expert-tiering before
        // the sidecar is loaded (tensor_split, loader overrides, etc.).  Those
        // decisions are model-specific and must not leak into a tiny MTP model.
        // Start the draft placement from clean single-model defaults, then apply
        // only the explicit speculative-draft knobs above.
        std::fill(std::begin(result.tensor_split), std::end(result.tensor_split), 0.0f);
        result.main_gpu   = 0;
        result.split_mode = LLAMA_SPLIT_MODE_LAYER;
        result.fit_params = false;
        result.cmoe       = false;
...
common_speculative_init_result::common_speculative_init_result(
    ...
    llama_context * ctx_tgt,
      llama_model * model_dft_preloaded) :
    pimpl(new impl{}) {
    const bool has_draft = params.speculative.has_dft();
    const bool spec_mtp = std::find(params.speculative.types.begin(),
                          ...
                          std::find(params.speculative.types.begin(),
                                    params.speculative.types.end(),
                                    COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE) != params.speculative.types.end();
    GGML_ASSERT(has_draft || spec_mtp);

    auto mparams = common_model_params_to_llama(params);
    auto cparams = common_context_params_to_llama(params);

    // Tree verification branches live only in the target context. The MTP
    // sidecar itself still drafts one logical sequence per server slot.
    // Reusing the inflated target n_seq_max here wastes context and, worse,
    // makes output_reserve() require more rows than the draft context allocated.
    ...
    if (has_draft) {
        ...
        llama_model * model_dft = model_dft_preloaded;
        if (model_dft) {
            LOG_INF("%s: using preloaded draft model '%s'\n", __func__, model_path.c_str());
        } else {
...
common_speculative_init_result_ptr common_speculative_init_from_params(common_params & params, llama_model * model_tgt, llama_context * ctx_tgt, llama_model * model_dft_preloaded) {
    return std::make_unique<common_speculative_init_result>(params, model_tgt, ctx_tgt, model_dft_preloaded);
}
...
common_speculative * common_speculative_init(common_params_speculative & params, uint32_t n_seq) {
    ...
    std::vector<common_speculative_config> configs = {}; // list of speculative configs to try
    {
        ...
        bool has_draft_mtp_tree = (enabled_configs & (1u << COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE)) && params.draft.ctx_dft != nullptr;
        ...
        if (has_draft_mtp_tree) {
            configs.push_back(common_speculative_config(COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE, params));
        }
        if (has_draft_dflash) {
    ...
    for (const common_speculative_config & config : configs) {
        switch (config.type) {
            ...
            case COMMON_SPECULATIVE_TYPE_DRAFT_MTP_TREE: {
                impls.push_back(std::make_unique<common_speculative_impl_draft_mtp>(config.params, n_seq, true));
                break;
            }
            case COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH: {
...
void common_speculative_accept_at(common_speculative * spec, llama_seq_id seq_id, uint16_t n_accepted, int32_t verify_batch_index) {
    common_speculative_impl * impl = spec->impl_last[seq_id];
...

    {
        ...
        impl->accept_at(seq_id, n_accepted, false, verify_batch_index);
        impl->n_call_accept++;
    }
