    ...
    for (int64_t iir1 = ir1_start; iir1 < ir1_end; iir1 += blck_1) {
        for (int64_t iir0 = ir0_start; iir0 < ir0_end; iir0 += blck_0) {
            for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir1_end; ++ir1) {
                ...
                const int64_t row_base = matrix_row_offsets ? matrix_row_offsets[cur_a] : cur_a*ids->ne[0]*ids->ne[1];
                struct mmid_row_mapping row_mapping = matrix_rows[row_base + _i12];
    ...
    for (int cur_a = 0; cur_a < n_as; ++cur_a) {
        ...
        while (current_chunk < nchunk0 * nchunk1) {
            ...
            ggml_compute_forward_mul_mat_id_one_chunk(
                ...
                ir0_start, ir0_end, ir1_start, ir1_end,
                src0_cur, matrix_rows, NULL, row_size, src1_cont, wdata
            );
...
static void ggml_compute_forward_mul_mat_id_cold(
    ...
    int64_t * matrix_row_offsets = // [n_as]
        incr_ptr_aligned(&wdata_cur, n_as*sizeof(int64_t), sizeof(int64_t));

    int64_t * matrix_row_write = // [n_as]
        incr_ptr_aligned(&wdata_cur, n_as*sizeof(int64_t), sizeof(int64_t));

    struct mmid_row_mapping * matrix_rows = // [ids->ne[0]*ids->ne[1]]
        incr_ptr_aligned(&wdata_cur, ids->ne[0]*ids->ne[1]*sizeof(struct mmid_row_mapping), sizeof(int64_t));

    const char ** expert_src = // [n_as]
        (const char **) incr_ptr_aligned(&wdata_cur, n_as*sizeof(const char *), sizeof(int64_t));
    ...
    if (ith == 0) {
        ...
        // Prefix sums make one compact bucket per expert while preserving the
        // original token/slot order inside each bucket.
        int64_t row_off = 0;
        for (int i02 = 0; i02 < n_as; ++i02) {
            matrix_row_offsets[i02] = row_off;
            matrix_row_write[i02] = row_off;
            row_off += matrix_row_counts[i02];
        }
        for (int64_t iid1 = 0; iid1 < ids->ne[1]; ++iid1) {
            for (int id = 0; id < n_ids; ++id) {
                const int32_t i02 = *(const int32_t *) ((const char *) ids->data + iid1*ids->nb[1] + id*ids->nb[0]);
                ...
                matrix_rows[matrix_row_write[i02]++] = (struct mmid_row_mapping) {id, iid1};
            }
        }
    }
    ...
    for (int cur_a = ith; cur_a < n_as; cur_a += nth) {
        expert_src[cur_a] = matrix_row_counts[cur_a] > 0 ?
            moe_cold_addr(ptrs, addr, cur_a, (const char *) src0->data + cur_a * nb02) : NULL;
    }
    ...
    for (int cur_a = 0; cur_a < n_as; ++cur_a) {
        ...
        const char * src0_cur = expert_src[cur_a];
        const void * wdata = (src1->type == vec_dot_type) ? src1->data : params->wdata;
        const size_t row_size = ggml_row_size(vec_dot_type, ne10);
...
static uint64_t g_moe_cold_mmid_us = 0;

uint64_t ggml_moe_cold_timer_us(void) {
    return g_moe_cold_us;
}

uint64_t ggml_moe_cold_mmid_timer_us(void) {
    return g_moe_cold_mmid_us;
}
...
static void ggml_compute_forward_moe_cold(
    ...
    int64_t * matrix_row_write =
        (int64_t *) incr_ptr_aligned(&wdata_cur, n_as*sizeof(int64_t), sizeof(int64_t));

    struct mmid_row_mapping * matrix_rows =
        (struct mmid_row_mapping *) incr_ptr_aligned(&wdata_cur, (int64_t)n_ids*n_tokens*sizeof(struct mmid_row_mapping), sizeof(int64_t));
    ...
    if (ith == 0) {
        ...
        for (int64_t t = 0; t < n_tokens; t++) {
            for (int id = 0; id < n_ids; id++) {
                ...
                matrix_rows[matrix_row_write[e]++] = (struct mmid_row_mapping) {id, (int32_t) t};
            }
        }
    }
    ...
    for (int cur_a = 0; cur_a < n_as; ++cur_a) {
        ...
        while (current_chunk < nchunk0*nchunk1) {
            ...
            for (int64_t c = ir1_start; c < ir1_end; c++) {
                const struct mmid_row_mapping rm = matrix_rows[col0[cur_a] + c];
                ...
                case GGML_OP_MUL_MAT_ID_COLD:
                    {
                        ...
                        if (node->op == GGML_OP_MUL_MAT_ID_COLD) {
                            // compact cold buckets: offsets + write cursors + one row per selected pair + resolved expert sources
                            cur += 2*n_as*sizeof(int64_t) + 2*sizeof(int64_t);
                            cur += ids->ne[0]*ids->ne[1]*sizeof(struct mmid_row_mapping) + sizeof(int64_t);
                            cur += n_as*sizeof(const char *) + sizeof(int64_t);
                        } else {
                            // stock layout keeps one fixed-capacity bucket per expert
                ...
                case GGML_OP_MOE_COLD:
                    {
                        ...
                        // matrix_row_counts + compact write cursors + col0
                        cur += 3*n_as*sizeof(int64_t) + 3*sizeof(int64_t);
                        // matrix_rows: one entry per routed pair
                        cur += maxc*sizeof(struct mmid_row_mapping) + sizeof(int64_t);
...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n; node_n++) {
        ...
        const int64_t t_cold_0 = (state->ith == 0 &&
                (node->op == GGML_OP_MOE_COLD || node->op == GGML_OP_MUL_MAT_ID_COLD)) ? ggml_time_us() : 0;
        ...
        if (t_cold_0) {
            const uint64_t dt = (uint64_t)(ggml_time_us() - t_cold_0);
            g_moe_cold_us += dt;
            if (node->op == GGML_OP_MUL_MAT_ID_COLD) {
                g_moe_cold_mmid_us += dt;
            }
        }
    }
