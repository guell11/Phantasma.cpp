...
static ggml_cuda_device_info ggml_cuda_init() {
    ...
    for (int id = 0; id < info.device_count; ++id) {
        ...
        info.devices[id].cc_major          = prop.major;
        info.devices[id].cc_minor          = prop.minor;
        info.devices[id].max_threads_sm    = prop.maxThreadsPerMultiProcessor;
        info.devices[id].max_threads_block = prop.maxThreadsPerBlock;
        info.devices[id].regs_sm           = prop.regsPerMultiprocessor;
        info.devices[id].smem_sm           = prop.sharedMemPerMultiprocessor;
        info.devices[id].l2_bytes          = prop.l2CacheSize;
        info.devices[id].async_engines     = prop.asyncEngineCount;
        if (prop.major >= 7) {
            info.devices[id].tensor_core_modes |= GGML_BACKEND_CUDA_TENSOR_CORE_F16;
        }
        if (prop.major >= 8) {
            info.devices[id].tensor_core_modes |= GGML_BACKEND_CUDA_TENSOR_CORE_BF16 | GGML_BACKEND_CUDA_TENSOR_CORE_TF32;
        }
        if (prop.major > 8 || (prop.major == 8 && prop.minor >= 9)) {
            info.devices[id].tensor_core_modes |= GGML_BACKEND_CUDA_TENSOR_CORE_FP8;
        }
        GGML_LOG_INFO("  Device %d: %s, compute capability %d.%d, VMM: %s, VRAM: %zu MiB\n",
                      id, prop.name, prop.major, prop.minor, device_vmm ? "yes" : "no",
                      device_vram_mib);
        ...
        if (device_name == "NVIDIA GeForce MX450") {
            ...
            info.devices[id].tensor_core_modes = 0;
...
static bool ggml_backend_cuda_get_device_capabilities(
        ggml_backend_dev_t dev,
        ggml_backend_cuda_device_capabilities * capabilities) {
    if (!dev || !capabilities) {
        return false;
    }
    ...
    ggml_backend_cuda_device_context * ctx = (ggml_backend_cuda_device_context *) dev->context;
    const ggml_cuda_device_info & info = ggml_cuda_info();
    if (ctx->device < 0 || ctx->device >= info.device_count) {
        return false;
    }
    ...
    *capabilities = {
        device.cc_major,
        device.cc_minor,
        static_cast<uint32_t>(device.warp_size),
        device.max_threads_sm,
        device.max_threads_block,
        device.regs_sm,
        device.smem_sm,
        device.l2_bytes,
        device.async_engines,
        device.tensor_core_modes,
    };
    return true;
}
#endif
...
#if !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA)
    if (strcmp(name, "ggml_backend_cuda_get_device_capabilities") == 0) {
        return (void *) ggml_backend_cuda_get_device_capabilities;
    }
#endif
    return nullptr;
}
